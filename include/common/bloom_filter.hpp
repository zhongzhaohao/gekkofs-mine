...
public:
   ...
   std::string serialize() const {
      ...
      uint64_t salt_size = salt_.size();
      append_binary(binary_data, salt_size);
      append_binary(binary_data, salt_.data(), salt_size * sizeof(bloom_type));
      
      uint64_t bit_table_size = bit_table_.size();
      append_binary(binary_data, bit_table_size);
      append_binary(binary_data, bit_table_.data(), bit_table_size);
      
      append_binary(binary_data, salt_count_);
      append_binary(binary_data, table_size_);
      append_binary(binary_data, projected_element_count_);
      append_binary(binary_data, inserted_element_count_);
   ...
   void deserialize(const std::string& str) {
      ...
      projected_element_count_ = read_binary<unsigned long long>(ptr);
      ptr += sizeof(unsigned long long);
      inserted_element_count_ = read_binary<unsigned long long>(ptr);
      ptr += sizeof(unsigned long long);
      random_seed_ = read_binary<unsigned long long>(ptr);
      ptr += sizeof(unsigned long long);
   ...
   template<typename T>
   static void append_binary(std::vector<unsigned char>& buffer, const T& value) {
      const unsigned char* p = reinterpret_cast<const unsigned char*>(&value);
      buffer.insert(buffer.end(), p, p + sizeof(T));
   }

   static void append_binary(std::vector<unsigned char>& buffer, const void* data, size_t size) {
      const unsigned char* p = static_cast<const unsigned char*>(data);
      buffer.insert(buffer.end(), p, p + size);
   }

   template<typename T>
   static T read_binary(const unsigned char*& ptr) {
      T value;
